...
MAX_STEPS_PER_EPISODE = 30 # Prevent infinitely long episodes
...
def run_simulation(
    ...
    while len(episode_rewards) < min_num_episodes or global_iter < min_num_iters:
        ...
        # print("check terminal", env.is_terminal((2, 4)))
        ...
        # if env.is_terminal
        # if env.is_terminal(s1):
        #     print("reached terminal", s1)
        # else:
        #     print("not reached", s1)
...
def generate_instances(basemaze, sample_size=100, lamb=1, demonstrate_lamb=1, discount=0.95, seed=0, noise=0, num_trajectories=1000, action_error_prob=0):
    ...
    desired_path = [(4,0),(4,1),(4,2),(4,3),(3,3),(2,3),(1,3),(0,3)]
    full_labels = []
    full_features = []
    maze_list = []
    dataset = []
    ...
    for sample_id in range(sample_size):
        ...
        for x1 in range(maze.shape[0]):
            for x2 in range(maze.shape[1]):
                ...
                if maze.topology[x1,x2] == '$':
                    reward_function[x1,x2] = np.random.normal(loc=10, scale=1)
                    feature_function[x1,x2,0] = np.random.normal(loc=9, scale=1)
                else:
                    # # code for desired  path
                    # if (x1, x2) in desired_path:
                    #     desired_path_ind = desired_path.index((x1, x2))
                    #     # print(desired_path_ind)
                    #     reward_function[x1,x2] = np.random.normal(loc=x2 + desired_path.index((x1, x2)) * 0.5, scale = 1)
                    #     feature_function[x1,x2,0] = np.random.normal(loc=x2 + 1, scale = 1)
                    # else:
                    #     reward_function[x1,x2] = -5
                    #     feature_function[x1,x2,0] = -5

                    # random features and no rewards
                    # reward_function[x1, x2] = 0
                    # feature_function[x1,x2,0] = np.random.normal(loc = 5, scale = 5)

                    #
                    if x2 > 4 and x1 < 3:
                        reward_function[x1,x2] = np.random.normal(loc = 3, scale = 0.5)
                        feature_function[x1,x2,0] = np.random.normal(loc = 3, scale = 0.5)
                    else:
                        reward_function[x1, x2] = 0
                        feature_function[x1,x2,0] = np.random.normal(loc = 0, scale = 0.5)
        ...
        reward_function = torch.clip(reward_function, min=-10, max=25)
        # if noise_size > 0:
