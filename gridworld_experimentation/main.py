...
def save_Q(Q, a):
    # convert to numpy
    numpy_Q = Q.detach().numpy()
    for i in range(len(numpy_Q)):
        df = pd.DataFrame(numpy_Q[i])
        df.to_csv(a + "Q" + str(i) + ".csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--noise', type=float, default=0.0, help='noise std added to the generated features')
    ...
    def generate_custom_maze():
        # create custom maze with walls
        maze = ['....$', '..#..', '#.#.#', '..#..', 'o....']
        return maze
    # basemaze = Maze(generate_maze_description(problem_size))

    def generate_custom_maze_2():
        maze = ['....#..$','....#...', '........','##.###.#', '....#...', '........', 'o...#...']
        return maze
    basemaze = Maze(generate_custom_maze_2())
    ...
    print("num features: ", feature_size)
        ...
        for mode, data_loader in [('train', train_loader), ('validate', validate_loader), ('test', test_loader)]:
            ...
            with tqdm.tqdm(data_loader) as tqdm_loader:
                for index, (data_id, maze, feature, label, real_trajectories) in enumerate(tqdm_loader):
                    ...
                    print("cur features: ", feature)
                    print("len features: ", feature.size())
                    ...
                    print("cur label: ", label)
                    print("cur pred: ", prediction)
                    print("traj: ", real_trajectories)
