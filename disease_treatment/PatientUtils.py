...
def generate_instances(
    ...
    discount,
    env_type,
    sample_size=1, # prev default = 10
    softness=1,
    demonstrate_softness=100,
    num_trajectories=50, # prev default = 100
    seed=0,
    noise_fraction=0,
    feature_size=16,
    noise_dimensions=0,
    ...
    # Transition Matrix: (NUM_MEDICINES * NUM_STATES=2 * NUM_STATES=2)
    num_targets = NUM_MEDICINES
    target_size = 6
    ...
    for sample_id in tqdm.tqdm(range(sample_size)):
        ...
        env = env_type(NUM_STATES=NUM_STATES, NUM_MEDICINES = NUM_MEDICINES, DATA_PATH=data_file)
        transition_prob = env.true_patient_models.to(torch.float)
        # print(transition_prob.shape)
        # print(transition_prob.view(num_targets, target_size))
        # print(channel_size_list)
        ...
        dataset.append([sample_id, NUM_STATES, NUM_MEDICINES, feature, transition_prob, trajectories])
...
if __name__ == '__main__':
    from PatientWorld import PatientWorld
    dataset, d = generate_instances(2, 4, 0.9, PatientWorld)